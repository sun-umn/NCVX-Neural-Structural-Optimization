...
def build_structure_comparison_data(name: str) -> pd.DataFrame:
    ...
    for experiment in experiment_prefix_list:
        filename = f"{experiment}_{name}.csv"
    ...
    google_filename = f"google_{name}.csv"
    google_results = pd.read_csv(os.path.join(path, "results", google_filename))  # noqa
    ...
    pygranso_filename = f"pygranso_{name}.csv"
    pygranso_results = pd.read_csv(
        os.path.join(path, "results", pygranso_filename)
    )  # noqa
    ...
    results = pd.merge(pygranso_results, google_results, on="step", how="left").ffill()  # noqa
