...
from str_opt_utils import models, problems, topo_api, topo_physics
...
def structural_optimization_function(
    model, z, freedofs_forces, ke, args, designs, kwargs, opt_x_phys, debug, x_phys
):  # ,u,K):
    """
    ...
    K, _ = topo_physics.get_KU(x_phys, ke, args["freedofs"], args["fixdofs"], **kwargs)
    ...
    f = (
        torch.sum(u @ freedofs_forces) * f_factor
        + (torch.mean(x_phys) - args["volfrac"]) * 0
    )
...
cnn_model = models.CNNModel(args=args, **cnn_kwargs).to(
...
fixed_random_input = torch.normal(
    mean=torch.zeros((1, 128)), std=torch.ones((1, 128))
...
_, index_map = topo_physics.get_KU(
    torch.ones((20, 60)).to(device=device, dtype=double_precision),
    ke,
    args["freedofs"],
    args["fixdofs"],
    **kwargs,
